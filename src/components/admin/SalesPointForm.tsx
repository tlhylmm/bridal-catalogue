...
export default function SalesPointForm({ initialData }: SalesPointFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            let result;
            if (initialData) {
                result = await updateSalesPoint(initialData.id, formData);
            } else {
                result = await createSalesPoint(formData);
            }

            if (result.success) {
                showToast(`'${name}' satış noktası başarıyla ${initialData ? 'güncellendi' : 'eklendi'}!`, 'success');
            ...
            console.error('Satış noktası kaydedilirken hata:', err);
            showToast(`'${name}' satış noktası kaydedilirken hata oluştu!`, 'error');
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <>
            <form onSubmit={handleSubmit} className="admin-form" style={{ maxWidth: '800px' }}>
                ...
                <button type="submit" className="admin-btn admin-btn-primary" disabled={submitting}>
                    {submitting ? 'Kaydediliyor...' : (initialData ? 'Satış Noktasını Güncelle' : 'Satış Noktası Oluştur')}
                </button>
            </form>
        </>
    );
